 ...
 *      Author: Alejandro Mera ft. Paul Kutukov 2023
...
void ModbusStart(modbusHandler_t *modH)
{
	...
	{
		puts("\tERR: To use USART_HW_DMA you need to enable it in the ModbusConfig.h");
		while (1)
	...
	if (modH->xTypeHW == USART_HW || modH->xTypeHW == USART_HW_DMA)
	{
		...
		{
			puts("\tERR: Shared mem pointer is NULL");
			while (1)
		...
		if (modH->xTypeHW == USART_HW_DMA)
		{
			...
			{
				puts("\tERR: Failed to init DMA req");
				while (1)
				{
		...
		else
		{
			...
			{
				puts("\tERR: Failed to init UART IT recv");
				while (1)
				{
					// error in your initialization code
				}
		...
		{
			puts("\tERR: Slave ID can not be 0");
			while (1)
			{
				// error Master ID must be zero
			}
